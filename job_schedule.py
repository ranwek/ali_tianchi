...
def update1(key,update_max):
    ...
    if job_max[key]>=update_max:
        ...
        for predecessor_key in job_dict[key][5:]:
            if job_dict.get(predecessor_key)!=None:
                # deploy predecessor
                update1(predecessor_key,job_max[key]-int(100+job_time))
                job_min[key]=job_max[key]-int(100+job_time)
...
def update2(key,update_max):
    ...
    if job_max[key]>=update_max:
        ...
        for predecessor_key in job_dict[key][5:]:
            if job_dict.get(predecessor_key)!=None:
                # deploy predecessor
                update2(predecessor_key,job_max[key]-int(200+job_time))
                job_min[key]=job_max[key]-int(200+job_time)
...
def deploy(key):
    ...
    if job_deploy.get(key)==None:
        ...
        for add_round in range(5):
            if job_n>0:
                print(job_max[key],job_min[key])
                for i in range(len(deploy_machine)):
                    if machine_num[machine_number[deploy_machine[i]]]>5:
                        continue
                    #找到可以部署的合适的位置
                    point=job_min[key]+np.argmax(mydot_individual(machine_resources_cpu_job[machine_number[deploy_machine[i]],job_min[key]:job_max[key]]+0.1*(add_round+1)*machine_resources_data.iloc[machine_number[deploy_machine[i]]]['cpu0'],job_time,job_cpu))
                    #该位置最大部署数量
                    max_deploy_job=int(min(np.min((machine_resources_cpu_job[machine_number[deploy_machine[i]],point:point+job_time]+0.1*(add_round+1)*machine_resources_data.iloc[machine_number[deploy_machine[i]]]['cpu0'])/job_cpu),\
                                       np.min((machine_resources_mem_job[machine_number[deploy_machine[i]],point:point+job_time]))/job_mem))
                    if max_deploy_job>=job_n:
                        #如果可以部署
                        if job_n>0:
                ...
                break
        if job_n>0:
            for i in range(len(undeploy_machine)):
                #找到可以部署的合适的位置
                point=job_min[key]+np.argmax(mydot_individual(machine_resources_cpu_job[machine_number[undeploy_machine[i]],job_min[key]:job_max[key]],job_time,job_cpu))
                #该位置最大部署数量
                max_deploy_job=int(min(np.min(machine_resources_cpu_job[machine_number[undeploy_machine[i]],point:point+job_time]/job_cpu),\
                                   np.min(machine_resources_mem_job[machine_number[undeploy_machine[i]],point:point+job_time]/job_mem)))
                if max_deploy_job>=job_n:
                    #如果可以部署
                    if job_n>0:
                        machine_resources_cpu_job[machine_number[undeploy_machine[i]],point:point+job_time]-=job_n*job_cpu
                        machine_resources_mem_job[machine_number[undeploy_machine[i]],point:point+job_time]-=job_n*job_mem
                        print(job_name+',machine_'+str(int(undeploy_machine[i]))+','+str(point)+','+str(job_n),file=file_reulst)
                        job_n=0
                        break
                else:
                    #如果可以部署
                    if max_deploy_job>0:
                        machine_resources_cpu_job[machine_number[undeploy_machine[i]],point:point+job_time]-=max_deploy_job*job_cpu
                        machine_resources_mem_job[machine_number[undeploy_machine[i]],point:point+job_time]-=max_deploy_job*job_mem
                        print(job_name+',machine_'+str(int(undeploy_machine[i]))+','+str(point)+','+str(max_deploy_job),file=file_reulst)
                        job_n-=max_deploy_job
        if job_n>0:
            print('no_capacity')
...
if __name__ == '__main__':    
    ...
    for each_job_key in job_dict.keys():
        if job_dict.get(job_dict[each_job_key][5])!=None:
            #deploy predecessor
            if int(each_job_key.split('-')[1])>7:
                update(each_job_key,1470)
            elif int(each_job_key.split('-')[1])>5:
                update1(each_job_key,1470)
            else:
                update2(each_job_key,1470)

    for each_job_key in job_dict.keys():
        if job_min[each_job_key]==0 and job_max[each_job_key]>500:
            job_max[each_job_key]=500
    ...
    undeploy_machine=machine_resources_data['machine_id'][~machine_resources_data['machine_id'].isin(deploy_machine)].values
    ...
    deploy_machine=np.hstack((undeploy_machine[0:400],deploy_machine))
